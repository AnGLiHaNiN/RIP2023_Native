//
//  DetailInformationViewController2.swift
//  yourProjectName
//
...
class DetailInformationViewController2: UIViewController {
...
//MARK: - methods
extension DetailInformationViewController2 {
    func setupView() {
        NetworkService.shared.getOneComponent(componentID: uuid) { result in
                    ...
                    NetworkService.shared.loadImage(from: url) { imageNew in
                        DispatchQueue.main.async {
                            if let image = imageNew {
                                ...
                                self.descriptionaLabel.text = data.properties
                                self.factoryLabel.text = data.worldName
                                self.photoImageView.image = image
                            } else {
                                print("Нет изображения для ")
                            }
                            group.leave()
                        }
                    }
...
private extension DetailInformationViewController2 {
